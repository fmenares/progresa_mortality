...
preserve
	...
	gen byte in_enigh2000 = (year == 2000)
	collapse (max) in_enigh1998 in_enigh2000, by(cve_ent_mun_super)
	save "$data/enigh_mun_list_all.dta", replace
restore
...
local years = "1997 1998 2000"
* -- FA0b: box plot by ENIGH survey wave --
foreach year in `years' {
preserve
	keep if inten`year' != .
	bysort cve_ent_mun_super year: keep if _n == 1
	graph box inten`year', over(year, label(angle(45) labsize(medium)))         ///
	ytitle("PROGRESA Intensity")                          ///
		graphregion(fcolor(white)) 
	graph export "$figures/appendix/Figure_inten`year'_bywave.pdf", replace
restore
}
...
* Flag municipalities observable in ENIGH 1998 and 2000 waves
* (enigh_mun_list.dta is built by 03_descriptives.do — run that first)
merge 1:1 cve_ent_mun_super using "$data/enigh_mun_list_all.dta", ///
	keepusing(in_enigh1998 in_enigh2000) nogen
recode in_enigh1998 in_enigh2000 (. = 0)
gen inten1998_enigh = inten1998 if in_enigh1998 == 1
gen inten2000_enigh = inten2000 if in_enigh2000 == 1
...
spmap inten1998_enigh using "${shp}\municipios_2000_shp.dta", id(_ID) ///
	...
	legend(size(small) position(7)) ///
	graphregion(fcolor(white))
graph export "$figures/maps/Figure_2c_enigh_inten1998.pdf", as(pdf) replace
...
spmap inten2000_enigh using "${shp}\municipios_2000_shp.dta", id(_ID) ///
	...
	fcolor(Blues2) ocolor(none ..) osize(vvthin ..) ///
	graphregion(fcolor(white))
graph export "$figures/maps/Figure_2d_enigh_inten2000.pdf", as(pdf) replace
*/
...
spmap inten1997_enigh using "${shp}\municipios_2000_shp.dta", id(_ID) ///
	...
	graphregion(fcolor(white))
graph export "$figures/maps/Figure_2f_inten1997_enigh.pdf", as(pdf) replace
*/
restore
...
spmap inten1998_enigh using "${shp}\municipios_2000_shp.dta", id(_ID) ///
	...
	graphregion(fcolor(white))
graph export "$figures/maps/Figure_2c_enigh_inten1998_all.pdf", as(pdf) replace
...
spmap inten2000_enigh using "${shp}\municipios_2000_shp.dta", id(_ID) ///
	...
	legend(size(medium) position(7)) ///
	graphregion(fcolor(white))
graph export "$figures/maps/Figure_2d_enigh_inten2000_all.pdf", as(pdf) replace
...
spmap inten1997_enigh using "${shp}\municipios_2000_shp.dta", id(_ID) ///
	...
	legend(size(medium) position(7)) ///
	graphregion(fcolor(white))
graph export "$figures/maps/Figure_2f_inten1997_enigh_all.pdf", as(pdf) replace
