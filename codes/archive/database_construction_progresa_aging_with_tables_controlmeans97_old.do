...
//agric variables
 ...
 bys folio ronda: egen n_adults = total(inrange(age,15,64))
 bys folio ronda: egen n_elderly = total(age >=65)
 bys folio ronda: egen n_children = total(age < 15)
...
local indiv_outcomes work days_week hours_day live_alone 
...
esttab work_all work_elig ///
      days_week_all days_week_elig ///
       ...
       live_alone_all live_alone_elig ///
...
foreach yvar of local hh_outcomes {
    ...
    * (reported as the overall control mean in 1997; the heterogeneity coefficients are differences relative to this baseline)
     summarize `yvar' if year==98 & contba==0 & !missing(`yvar', year, contba, hh_elderly, hhid, claveofi)
    local cmean97_het = r(mean)

    * Heterogeneity: households with elderly (elderly97) vs without
     reghdfe `yvar' i.year##i.contba##i.hh_elderly ///
        if !missing(`yvar', year, contba, hh_elderly, hhid, claveofi), ///
        absorb(clavemun) ///
        vce(cluster claveofi)
    ...
    * Control mean in 1997 for eligible households (heterogeneity regression)
     summarize `yvar' if year==98 & contba==0 & eligible==1 & !missing(`yvar', year, contba, hh_elderly, hhid, claveofi)
    local cmean97_hetelig = r(mean)

     reghdfe `yvar' i.year##i.contba##i.elderly97 ///
        if eligible==1 & !missing(`yvar', year, contba, hh_elderly, hhid, claveofi), ///
        absorb(clavemun) ///
        vce(cluster claveofi)
...
esttab food_het food_hetelig medicine_het medicine_hetelig ///
    ...
    keep(99.year#1.contba 99.year#1.contba#1.hh_elderly) ///
    order(99.year#1.contba 99.year#1.contba#1.hh_elderly) ///
    coeflabels(99.year#1.contba "Treatment x 1999 (no elderly)" ///
               99.year#1.contba#1.hh_elderly "Diff: elderly hh x 1999") ///
    b(%9.3f) se(%9.3f) ///
    star(* 0.10 ** 0.05 *** 0.01) ///
    stats(N cmean97, fmt(%9.0g %9.3f) labels("Observations" "Control mean (1997)")) ///
    compress
