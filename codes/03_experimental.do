...
*pooled — age 51+ (Gertler 2000 comparison)*
local g p
preserve
keep if age97>=51
...
if r(N) > 0 {
    foreach ggrp in p m f {
        if "`ggrp'" == "p"      local gcond "age97>=51"
        else if "`ggrp'" == "m" local gcond "gender==1 & age97>=51"
        else                     local gcond "gender==2 & age97>=51"
...
*------------------------------------------------------------
* Panel D: Pooled, age 65+ (reference; very small sample)
*------------------------------------------------------------
local g p65
di "Panel D (age 65+) sample:"
count if age97>=65 & eligible==1
preserve
keep if age97>=65
foreach yvar of local indiv_outcomes {
    summarize `yvar' if year==97 & contba==0 & eligible==1 & !missing(`yvar', year, contba, claveofi)
    local cmean97_elig = r(mean)
    cap reghdfe `yvar' i.year##i.contba ///
        if eligible==1 & !missing(`yvar', year, contba, claveofi), ///
        absorb(clavemun) vce(cluster claveofi)
    if "`yvar'" == "weekly_hours" & _rc == 0 {
        foreach yr in 98 99 {
            local aux : di %9.3f _b[`yr'.year#1.contba]
            local tstat = abs(_b[`yr'.year#1.contba] / _se[`yr'.year#1.contba])
            if `tstat' >= 2.576      local b`yr'_`g'_wh = trim("`aux'") + "***"
...
* total visits col 5 for age 65+
local gcond65 "age97>=65"
cap reghdfe total_visits contba ///
    if year==99 & eligible==1 & `gcond65' ///
    & !missing(total_visits, contba, claveofi), ///
    absorb(clavemun) vce(cluster claveofi)
if _rc == 0 {
    local aux : di %9.3f _b[contba]
    local tstat = abs(_b[contba] / _se[contba])
    if      `tstat' >= 2.576 local b99_p65_tv = trim("`aux'") + "***"
    else if `tstat' >= 1.960 local b99_p65_tv = trim("`aux'") + "**"
    else if `tstat' >= 1.645 local b99_p65_tv = trim("`aux'") + "*"
    else                     local b99_p65_tv = trim("`aux'")
    local se99_p65_tv : di %9.3f _se[contba]
    local N_p65_tv    : di %12.0fc e(N)
    quietly sum total_visits if year==99 & contba==0 & eligible==1 & `gcond65' & !missing(total_visits)
    local cmn_p65_tv : di %9.3f `r(mean)'
}

* diagnostics before writing table
di "--- T3 locals check (pooled 51+) ---"
di "b98_p_wh  = `b98_p_wh'  |  b99_p_wh  = `b99_p_wh'"
di "b98_p_la  = `b98_p_la'  |  b99_p_la  = `b99_p_la'"
di "b99_p_tv  = `b99_p_tv'"
di "--- T3 locals check (pooled 65+) ---"
di "b98_p65_wh = `b98_p65_wh' | b99_p65_wh = `b99_p65_wh'"
di "N_p_wh = `N_p_wh' | N_p65_wh = `N_p65_wh'"

*============================================================
* TABLE 3: Experimental results — Labor Supply and Living Arrangements
* Eligible households only; age 51+ (Panels A–C); age 65+ (Panel D)
...

{
    ...
    * Panel A: Pooled, age 51+
    file write sm "\multicolumn{6}{l}{\textbf{Panel A: Pooled (age 51+)}} \\ \midrule" _n
    ...
    * Panel B: Females, age 51+
    file write sm "\multicolumn{6}{l}{\textbf{Panel B: Females (age 51+)}} \\ \midrule" _n
    ...
    * Panel C: Males, age 51+
    file write sm "\multicolumn{6}{l}{\textbf{Panel C: Males (age 51+)}} \\ \midrule" _n
    ...
    file write sm "Control Mean (1999) & & & & & `cmn_m_tv' \\ \midrule" _n
    * Panel D: Pooled, age 65+ (reference)
    file write sm "\multicolumn{6}{l}{\textbf{Panel D: Pooled (age 65+, reference)}} \\ \midrule" _n
    file write sm "Treat \$\times\$ 1998 & `b98_p65_wh' & `b98_p65_la' & `b98_p65_wc' & `b98_p65_oe' & \\ " _n
    file write sm " & (`se98_p65_wh') & (`se98_p65_la') & (`se98_p65_wc') & (`se98_p65_oe') & \\[4pt]" _n
    file write sm "Treat \$\times\$ 1999 & `b99_p65_wh' & `b99_p65_la' & `b99_p65_wc' & `b99_p65_oe' & `b99_p65_tv' \\ " _n
    file write sm " & (`se99_p65_wh') & (`se99_p65_la') & (`se99_p65_wc') & (`se99_p65_oe') & (`se99_p65_tv') \\[4pt]" _n
    file write sm "Observations & `N_p65_wh' & `N_p65_la' & `N_p65_wc' & `N_p65_oe' & `N_p65_tv' \\ " _n
    file write sm "Control Mean (1997) & `cmn_p65_wh' & `cmn_p65_la' & `cmn_p65_wc' & `cmn_p65_oe' & \\ " _n
    file write sm "Control Mean (1999) & & & & & `cmn_p65_tv' \\ \bottomrule" _n
    file write sm "\multicolumn{6}{l}{\footnotesize \textsuperscript{\$\dagger\$} Total visits (hospital, public clinic, private doctor, past 4 weeks). 1999 only; cross-section, municipality FE.} \\" _n
