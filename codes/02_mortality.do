...
*============================================================
* INTENSITY CONSTRUCTION COMPARISON: build three parallel 1999/2005
* snapshots (inten1999/inten2005 above are left untouched, so every
* existing table/figure that references them is unaffected):
*   _orig : merged from the original $data/inten1999.dta/inten2005.dta
*           files (built once by the archived codes/archive/aamr_011326.do,
*           never regenerated by the current 00 -> 01 -> 02 pipeline).
*   _00   : same formula as inten1999/inten2005 above (intensity_new
*           frozen at that year), just duplicated under its own name so
*           it can sit next to _orig/_cum in the comparison below.
*   _cum  : c_pg_new (the cc_pg_mun cumulative-% ratio already built in
*           00.programs_beneficiaries_recoded.do and carried through
*           01_mortality_data.do untouched) frozen at that year -- the
*           genuine 1997-99/1997-05 cumulative-coverage measure Parker &
*           Vogl (2023) intended, as opposed to _orig/_00 which are both
*           intensity_new (admin-stock based).
*------------------------------------------------------------
	gen inten1999_00 = inten1999
	gen inten2005_00 = inten2005

	cap drop inten1999_orig inten2005_orig
	capture confirm file "$data/inten1999.dta"
	if _rc == 0 {
		preserve
		use "$data/inten1999.dta", clear
		rename inten1999 inten1999_orig
		keep cve_ent_mun_super inten1999_orig
		tempfile orig1999
		save `orig1999'
		restore
		merge m:1 cve_ent_mun_super using `orig1999', nogen
	}
	else {
		di as error "$data/inten1999.dta not found locally -- inten1999_orig set to missing"
		gen inten1999_orig = .
	}

	capture confirm file "$data/inten2005.dta"
	if _rc == 0 {
		preserve
		use "$data/inten2005.dta", clear
		rename inten2005 inten2005_orig
		keep cve_ent_mun_super inten2005_orig
		tempfile orig2005
		save `orig2005'
		restore
		merge m:1 cve_ent_mun_super using `orig2005', nogen
	}
	else {
		di as error "$data/inten2005.dta not found locally -- inten2005_orig set to missing"
		gen inten2005_orig = .
	}

	g aux = c_pg_new if year==1999
	bys cve_ent_mun_super: egen inten1999_cum = min(aux)
	drop aux
	g aux = c_pg_new if year==2005
	bys cve_ent_mun_super: egen inten2005_cum = min(aux)
	drop aux
...
* Compares the three 1999/2005 intensity constructions built above
* (_orig, _00, _cum) in the preferred T2 col-4 spec (weighted, +SP
* control, HM sample). _orig and _00 use the identical snapshot formula
* (intensity_new frozen at that year) so their correlation checks whether
* the current pipeline "basically matches" the original static files.
* _cum instead uses c_pg_new (genuine cumulative-% coverage from 00.).
* Output: $tables/appendix/AT_intensity_construction_comparison.tex
*============================================================
foreach v in orig 00 cum {
	local b99_cmp_`v'  ""
	local se99_cmp_`v' ""
	local b05_cmp_`v'  ""
	local se05_cmp_`v' ""
	...
	cap noisily reghdfe emr65 c.inten1999_`v'#i.post c.inten2005_`v'#i.post c.sp_intensity ///
		[aw=popover65_] if $sample_marg, a(year cve_ent_mun_super) vce(cluster cve_ent_mun_super)
	if _rc == 0 & e(N) > 0 {
		local b99_cmp_`v':  di %12.3f _b[1.post#c.inten1999_`v']
		local se99_cmp_`v': di %12.3f _se[1.post#c.inten1999_`v']
		local b05_cmp_`v':  di %12.3f _b[1.post#c.inten2005_`v']
		local se05_cmp_`v': di %12.3f _se[1.post#c.inten2005_`v']
...
corr inten1999_orig inten1999_00 if $sample_marg & year==1996
local corr99: di %5.3f r(rho)
corr inten2005_orig inten2005_00 if $sample_marg & year==1996
local corr05: di %5.3f r(rho)

{
	cap file close cmp
	file open cmp using "$tables/appendix/AT_intensity_construction_comparison.tex", write replace
	file write cmp "\begin{tabular}{lccc} \hline \hline" _n
	file write cmp "& \multicolumn{1}{c}{Original} & \multicolumn{1}{c}{00-Embedded} & \multicolumn{1}{c}{00-Embedded} \\ " _n
	file write cmp "& \multicolumn{1}{c}{(static files)} & \multicolumn{1}{c}{(snapshot)} & \multicolumn{1}{c}{(cumulative)} \\ " _n
	file write cmp "& \multicolumn{1}{c}{(1)} & \multicolumn{1}{c}{(2)} & \multicolumn{1}{c}{(3)} \\ \toprule" _n
	file write cmp "Intensity 1999 x post & `b99_cmp_orig' & `b99_cmp_00' & `b99_cmp_cum' \\ " _n
	file write cmp " & (`se99_cmp_orig') & (`se99_cmp_00') & (`se99_cmp_cum') \\ " _n
	file write cmp "  & & & \\ " _n
	file write cmp "Intensity 2005 x post & `b05_cmp_orig' & `b05_cmp_00' & `b05_cmp_cum' \\ " _n
	file write cmp " & (`se05_cmp_orig') & (`se05_cmp_00') & (`se05_cmp_cum') \\ " _n
	file write cmp "  & & & \\ " _n
...
* Output: $figures/appendix/Figure_2_orig.pdf / _00.pdf / _cum.pdf
*============================================================
{
local yr_labels `"1 "1991" 2 "1992" 3 "1993" 4 "1994" 5 "1995" 6 "1996" 7 "1997" 8 "1998" 9 "1999" 10 "2000" 11 "2001" 12 "2002" 13 "2003" 14 "2004" 15 "2005" 16 "2006""'
foreach v in orig 00 cum {
	cap noisily reghdfe emr65 c.inten1999_`v'##ib6.year_1995 c.inten2005_`v'##ib6.year_1995 ///
	...
	if _rc == 0 {
		forval pos = 1/16 {
			if `pos' == 6 {
				...
				local b99_`v'_`pos'  = _b[`pos'.year_1995#c.inten1999_`v']
				local se99_`v'_`pos' = _se[`pos'.year_1995#c.inten1999_`v']
				local b05_`v'_`pos'  = _b[`pos'.year_1995#c.inten2005_`v']
				local se05_`v'_`pos' = _se[`pos'.year_1995#c.inten2005_`v']
			}
		}
