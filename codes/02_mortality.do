...
*============================================================
* MDE AND EQUIVALENCE TEST (Comment 6)
* Reference regression: T2 col 4 — Pooled, Weighted + SP
* Three components:
*   (1) MDE at 80% and 90% power (alpha = 0.05, two-sided)
*   (2) 95% CI bounds and comparison to BR estimates
*   (3) TOST: two one-sided tests for negligible-effect range
*       delta = 2 deaths/1,000 (~4% of pre-program mean ~46)
* Output: $tables/appendix/AT_mde.tex
*============================================================

* Re-estimate T2 col 4 (pooled, weighted + SP) to recover scalar SE
quietly reghdfe emr65 c.inten1999#i.post c.inten2005#i.post c.sp_intensity ///
	[aw=popover65_] if $sample_marg, a(year cve_ent_mun_super) vce(cluster cve_ent_mun_super)

scalar b0    = _b[1.post#c.inten1999]
scalar se0   = _se[1.post#c.inten1999]

* (1) MDE
scalar mde80 = (1.960 + 0.842) * se0   // 80% power
scalar mde90 = (1.960 + 1.282) * se0   // 90% power

* (2) 95% CI
scalar ci_lo = b0 - 1.96 * se0
scalar ci_hi = b0 + 1.96 * se0

* (3) TOST with delta = 2 deaths/1,000
scalar delta  = 2
scalar t_lo   = (b0 + delta) / se0     // test H0: beta0 <= -delta
scalar t_hi   = (b0 - delta) / se0     // test H0: beta0 >= +delta
scalar p_tost = max(2 * (1 - normal(abs(t_lo))), 2 * (1 - normal(abs(t_hi))))
* TOST passes (reject "non-negligible") if p_tost < alpha = 0.05

* Pre-program mean (pooled, 65+, highly marginalized, 1991-1996)
quietly sum emr65 if $sample_marg & post == 2
scalar premean = r(mean)

* Display results
di _n "=== MDE & EQUIVALENCE (T2 col 4, Pooled W+SP) ==="
di "  beta0 = " %6.3f b0  "   SE = " %6.3f se0
di "  95% CI: [" %6.3f ci_lo ", " %6.3f ci_hi "]"
di "  MDE (80% power): " %6.3f mde80
di "  MDE (90% power): " %6.3f mde90
di "  Pre-program mean: " %6.2f premean
di "  MDE/mean (80%):   " %5.1f (mde80/premean)*100 "%"
di "  TOST delta=2: t_lo=" %5.3f t_lo "  t_hi=" %5.3f t_hi "  p=" %5.3f p_tost

* Format locals for table
local b0_f:     di %6.3f  b0
local se0_f:    di %6.3f  se0
local ci_lo_f:  di %6.3f  ci_lo
local ci_hi_f:  di %6.3f  ci_hi
local mde80_f:  di %6.3f  mde80
local mde90_f:  di %6.3f  mde90
local mean_f:   di %6.2f  premean
local mde80pct: di %4.1f  (mde80/premean)*100
local mde90pct: di %4.1f  (mde90/premean)*100
local tlo_f:    di %6.3f  t_lo
local thi_f:    di %6.3f  t_hi
local ptost_f:  di %6.3f  p_tost

{
	cap file close sm
	file open sm using "$tables/appendix/AT_mde.tex", write replace
	file write sm "\begin{tabular}{lc} \hline \hline" _n
	file write sm "\multicolumn{2}{l}{\textit{Reference: Pooled, Weighted + Seguro Popular (T2 col.\ 4)}} \\ \toprule" _n
	file write sm "\multicolumn{2}{l}{\underline{Point estimate and precision}} \\ " _n
	file write sm "Intensity 1999 x Post (\$\hat{\beta}_0\$) & `b0_f' \\ " _n
	file write sm "Standard error & (`se0_f') \\ " _n
	file write sm "95\% confidence interval & [`ci_lo_f', `ci_hi_f'] \\ " _n
	file write sm "Pre-program mean (1991--1996) & `mean_f' \\ " _n
	file write sm "  & \\ " _n
	file write sm "\multicolumn{2}{l}{\underline{Minimum detectable effect (two-sided, \$\alpha\$=0.05)}} \\ " _n
	file write sm "MDE at 80\% power & `mde80_f' \\ " _n
	file write sm "\% of pre-program mean & `mde80pct'\% \\ " _n
	file write sm "MDE at 90\% power & `mde90_f' \\ " _n
	file write sm "\% of pre-program mean & `mde90pct'\% \\ " _n
	file write sm "  & \\ " _n
	file write sm "\multicolumn{2}{l}{\underline{Two one-sided equivalence test (TOST), \$\delta\$=2 deaths/1,000}} \\ " _n
	file write sm "\$t\$-statistic (lower bound: \$H_0: \hat{\beta}_0 \leq -\delta\$) & `tlo_f' \\ " _n
	file write sm "\$t\$-statistic (upper bound: \$H_0: \hat{\beta}_0 \geq +\delta\$) & `thi_f' \\ " _n
	file write sm "TOST \$p\$-value (max of two one-sided \$p\$-values) & `ptost_f' \\ " _n
	file write sm "\bottomrule" _n
	file write sm "\multicolumn{2}{l}{\footnotesize \textit{Notes:} MDE = \$(z_{1-\alpha/2} + z_{\text{power}}) \times \text{SE}\$.} \\ " _n
	file write sm "\multicolumn{2}{l}{\footnotesize \$z_{0.975}=1.960\$, \$z_{0.842}=0.842\$ (80\%), \$z_{0.900}=1.282\$ (90\%).} \\ " _n
	file write sm "\multicolumn{2}{l}{\footnotesize BR original estimate: \$-6.37\$ (SE 1.04); BR weighted: \$-3.74\$.} \\ " _n
	file write sm "\multicolumn{2}{l}{\footnotesize TOST tests whether \$\hat{\beta}_0\$ lies within \$[-\delta,+\delta]\$; rejection} \\ " _n
	file write sm "\multicolumn{2}{l}{\footnotesize at \$\alpha=0.05\$ requires both one-sided \$p\$-values \$<0.05\$.} \\ " _n
	file write sm "\end{tabular}"
	file close sm
}
di "Table exported to: $tables/appendix/AT_mde.tex"
...
* Column 4: Drop bottom half (municipalities with pop_mun_br <= p50)
reghdfe emr65 lag2_intensity_new ///
    if inrange(year,1992,2002) & $sample_br & pop_mun_br > `p50_br', ///
    a(year cve_ent_mun_super) vce(cluster cve_ent_mun_super)
local aux: di %12.3f _b[lag2_intensity_new]
local t = abs(_b[lag2_intensity_new] / _se[lag2_intensity_new])
if      `t' >= 2.576 local bAT5_4 = "`aux'***"
else if `t' >= 1.96  local bAT5_4 = "`aux'**"
else if `t' >= 1.645 local bAT5_4 = "`aux'*"
else                  local bAT5_4 = "`aux'"
local seAT5_4:   di %12.3f _se[lag2_intensity_new]
sum emr65 if e(sample) & year == 1996
local meanAT5_4: di %12.2fc `r(mean)'
local NAT5_4:    di %12.0fc `e(N)'
distinct cve_ent_mun_super if e(sample)
...
*============================================================
* APPENDIX TABLE 6: Causes of Death (Highly Marginalized Sample, 1992-2002)
* AT6_cod_marg_short_mortality.tex -- Pooled, Female, Male panels
* Short-run spec: Intensity1999 x Post only (no Intensity2005 term)
*============================================================
foreach grp in w f m {
	if "`grp'" == "w" {
		local wvar = "popover65_"
	...
	foreach cod in tb_card tb_infect tb_diab tb_resp tb_nutri tb_cancer tb_accid tb_illdef tb_other {
		reghdfe emr65`cod'`suffix' c.inten1999#i.post c.sp_intensity ///
			[aw=`wvar'] if inrange(year,1992,2002) & $sample_marg, ///
			a(year cve_ent_mun_super) vce(cluster cve_ent_mun_super)
		local aux: di %12.3f _b[1.post#c.inten1999]
		local t = abs(_b[1.post#c.inten1999] / _se[1.post#c.inten1999])
		if      `t' >= 2.576 local b99_sh_`grp'_`cod' = "`aux'***"
		else if `t' >= 1.96  local b99_sh_`grp'_`cod' = "`aux'**"
		else if `t' >= 1.645 local b99_sh_`grp'_`cod' = "`aux'*"
		else                  local b99_sh_`grp'_`cod' = "`aux'"
		local se99_sh_`grp'_`cod': di %12.3f _se[1.post#c.inten1999]
		sum emr65`cod'`suffix' if e(sample) & post == 2
		local mean_sh_`grp'_`cod': di %12.2fc `r(mean)'
		local N_sh_`grp'_`cod': di %12.0fc `e(N)'
	}
}
...
*============================================================
* APPENDIX TABLE: Migration Robustness — Effect of Progresa
*   intensity on elderly population counts (65+).
*   Long-run window 1991-2006. Three functional forms in
*   columns (Levels, Log, Poisson); three panels (Pooled,
*   Male, Female).
*============================================================
...
foreach pnl in p m f {
	...
	* col 1: Levels (unweighted)
	...
	sum `outcome' if e(sample) & post == 0
	...
	* col 2: Log (unweighted)
	...
	sum `loutcome' if e(sample) & post == 0
...
*                   + gamma · (Intensity_2005 × Post × Old65)
*                   + Mun×Year + Mun×Age + Year×Age FE + e
*   Mun×Year FE absorbs overall municipality-year pop growth;
...
foreach pnl in p m f {
	...
	* col 1: Levels DDD
	reghdfe pop c.inten1999#i.post#i.old65 c.inten2005#i.post#i.old65 if $sample_marg, ///
		...
		vce(cluster cve_ent_mun_super)
	local aux: di %12.3f _b[1.post#1.old65#c.inten1999]
	local t = abs(_b[1.post#1.old65#c.inten1999] / _se[1.post#1.old65#c.inten1999])
	...
	local seDDD99_`pnl'_1: di %12.3f _se[1.post#1.old65#c.inten1999]
	local aux: di %12.3f _b[1.post#1.old65#c.inten2005]
	local t = abs(_b[1.post#1.old65#c.inten2005] / _se[1.post#1.old65#c.inten2005])
	...
	local seDDD05_`pnl'_1: di %12.3f _se[1.post#1.old65#c.inten2005]
	sum pop if e(sample) & post == 0 & old65 == 1
	...
	* col 2: Log DDD
	reghdfe lpop c.inten1999#i.post#i.old65 c.inten2005#i.post#i.old65 if $sample_marg, ///
		a(cve_ent_mun_super#year cve_ent_mun_super#age_grp year#age_grp) ///
		vce(cluster cve_ent_mun_super)
	local aux: di %12.3f _b[1.post#1.old65#c.inten1999]
	local t = abs(_b[1.post#1.old65#c.inten1999] / _se[1.post#1.old65#c.inten1999])
	...
	local seDDD99_`pnl'_2: di %12.3f _se[1.post#1.old65#c.inten1999]
	local aux: di %12.3f _b[1.post#1.old65#c.inten2005]
	local t = abs(_b[1.post#1.old65#c.inten2005] / _se[1.post#1.old65#c.inten2005])
	...
	local seDDD05_`pnl'_2: di %12.3f _se[1.post#1.old65#c.inten2005]
	sum lpop if e(sample) & post == 0 & old65 == 1
	...
	* col 3: Poisson DDD
	ppmlhdfe pop c.inten1999#i.post#i.old65 c.inten2005#i.post#i.old65 if $sample_marg, ///
		a(cve_ent_mun_super#year cve_ent_mun_super#age_grp year#age_grp) ///
		vce(cluster cve_ent_mun_super)
	local aux: di %12.3f exp(_b[1.post#1.old65#c.inten1999])-1
	local seDDD99_`pnl'_3: di %12.3f exp(_b[1.post#1.old65#c.inten1999])*_se[1.post#1.old65#c.inten1999]
	...
	local aux: di %12.3f exp(_b[1.post#1.old65#c.inten2005])-1
	local seDDD05_`pnl'_3: di %12.3f exp(_b[1.post#1.old65#c.inten2005])*_se[1.post#1.old65#c.inten2005]
