...
\begin{frame}{Mechanisms: Income, Nutrition, and Conclusions}\label{mechanisms_enigh}
\small

\textbf{Income \& nutrition (ENIGH, 1992--2006):} same two-phase DiD as main analysis
\begin{itemize}
...
\textbf{Takeaways across all mechanism tests:}
\begin{itemize}
  \item[$\circ$] \textbf{Labor supply \& living arrangements} (experimental): no significant changes $\to$ \textbf{caregiving pathway ruled out}
  \item[$\circ$] \textbf{Income, nutrition, health spending} (ENIGH): no detectable responses $\to$ \textbf{income/nutrition pathway not activated}
  \item[$\circ$] Two independent data sources converge: none of the channels through which \textit{Progresa} could reach the elderly were activated at detectable levels
  \item[$\circ$] The null mortality result is \textbf{interpretable}, not just a power issue
\end{itemize}

\end{frame}

\note{The ENIGH analysis finds no significant effects on labor income, food consumption, or health spending — consistent with the experimental null. Together, neither the caregiving nor the income and nutrition channels were activated. The null mortality result makes sense given these mechanism findings.}
