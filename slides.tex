...
\author[]{%
  \texorpdfstring{%
    \begin{tabular}{c c c}
      ...
      \textcolor{gray}{USC} & \textcolor{gray}{UC Berkeley} & \textcolor{gray}{IDB} \\[4pt]
...
\begin{itemize}
  \item[$\circ$] Income support programs and health: evidence is mixed
    \begin{itemize}
      \item Some studies find important health benefits \smallcite{Case2004PerspectivesAging, Aguila2015PNAS}
      \item Others find no effects \smallcite{LloydSherlockAgrawal2014JDS}
      \item Some identify unintended adverse consequences (diet change, obesity) \smallcite{FernaldGertlerHou2008JN}
    \end{itemize}
  \vspace{4pt}
  \item[$\circ$] Most existing evidence focuses on \textbf{short-run} outcomes; long-run \textbf{mortality} effects poorly understood
  \vspace{4pt}
  \item[$\circ$] CCTs have been widely studied for children, but \textbf{elderly} household members largely overlooked
    \begin{itemize}
      \item \smallcite{BehrmanParker2013Demography}: improved health-service utilization for elderly women
      \item \smallcite{aguila2024conditional}: higher hypertension diagnosis, but no improvement in biomarkers
      \item \smallcite{BarhamRowberry2013JDE}: \textit{short-run} reductions in elderly mortality over first 5 years
    \end{itemize}
  \vspace{4pt}
  \item[$\circ$] \textbf{Direct} pension programs for older adults show larger effects
    \begin{itemize}
      \item Mexico's \textit{70 y Más}: $-5.5\%$ elderly women mortality \smallcite{jeon2025short}
      \item Chile's basic pension: $-2.7$ pp four-year mortality \smallcite{MiglinoNavarrete2023AEJP}
    \end{itemize}
\end{itemize}
\end{frame}
...
\begin{itemize}
  ...
  \item[$\circ$] \textbf{Main finding:} \textcolor{redUT}{No evidence of medium- or long-run mortality effects}
    \begin{itemize}
      \item Result is robust to functional form, weighting, and Seguro Popular controls
    \end{itemize}
  \vspace{6pt}
  \item[$\circ$] \textbf{Revisiting prior evidence:} Short-run estimates of \smallcite{BarhamRowberry2013JDE} are sensitive to weighting and lag structure
    \begin{itemize}
      \item Minor, defensible specification changes attenuate estimates by $\geq 50\%$
    \end{itemize}
  \vspace{6pt}
...
\begin{columns}[T]
  \begin{column}{0.54\textwidth}
    \begin{itemize}
      \item[$\circ$] Launched 1997 in small rural communities; later renamed Oportunidades, then Prospera
      \vspace{4pt}
      \item[$\circ$] \textbf{Scale:} 6 million families covered ($\frac{1}{4}$ of all Mexican families); $\frac{2}{3}$ in communities $<$2,500 inhabitants
      \vspace{4pt}
      ...
      \item[$\circ$] \textbf{Conditionalities:}
        ...
        \end{itemize}
      \vspace{4pt}
      \item[$\circ$] \textbf{In 2006:} pension for adults $70+$ added --- we end analysis in 2006 to focus on original CCT design
    \end{itemize}
  \end{column}
  \begin{column}{0.44\textwidth}
    ...
    \begin{itemize}
      \item[$\circ$] \textbf{Elderly in eligible households:}
        ...
        \end{itemize}
      \vspace{8pt}
      \item[$\circ$] \textbf{Why would we expect an effect on the elderly?}
        \begin{itemize}
          \item Annual clinic visit could enable early detection of diabetes, hypertension, infections
          \item Household income increase could improve nutrition and healthcare access
          \item Changed co-residence $\to$ more informal caregiving
        \end{itemize}
    \end{itemize}
  \end{column}
\end{columns}
...
\begin{columns}[T]
  ...
  \begin{column}{0.48\textwidth}
    ...
    \vspace{4pt}
    \begin{itemize}
      \item[$\circ$] \textit{Health conditionality:} one annual visit is a minimal intervention; \smallcite{aguila2024conditional} find increased utilization but no improvement in blood pressure biomarkers
      \vspace{4pt}
      \item[$\circ$] \textit{Income:} transfers go to female head; elderly household members may not capture redistribution
      \vspace{4pt}
      \item[$\circ$] \textit{Caregiving:} \smallcite{BehrmanParker2013Demography} find physical-presence requirement for cash collection may actually \textit{increase} time demands on younger members
    \end{itemize}
    \vspace{8pt}
    \textbf{$\Rightarrow$ Whether channels are activated is an empirical question}
  \end{column}
\end{columns}
\end{frame}
...
\small
\begin{itemize}
  \item \textbf{Mortality data} $\rightarrow$ primary outcome
    \begin{itemize}
      \item Individual-level death registries from death certificates, 1991--2006 ($>3$ million records)
      \item Includes cause of death, birth year, sex, municipality of residence
      \item Publicly available from INEGI
    \end{itemize}
  \vspace{4pt}
  \item \textbf{Population data} $\rightarrow$ mortality rate denominators
    \begin{itemize}
      \item INEGI census counts by sex, age, municipality; linearly interpolated between census years
    \end{itemize}
  \vspace{4pt}
  ...
  \item \textbf{\textit{Progresa} experimental surveys (ENCASEH 1997; ENCEL 1998--2000)} $\rightarrow$ mechanisms
    ...
    \end{itemize}
  \vspace{4pt}
  \item \textbf{ENIGH (1992--2006)} $\rightarrow$ income and nutrition mechanisms
    \begin{itemize}
      \item National Household Expenditure Survey: food consumption, labor income, health spending
    \end{itemize}
\end{itemize}
\end{frame}
...
\begin{figure}
  \centering
  \includegraphics[width=0.82\textwidth, keepaspectratio]{Figure_1a_marg.pdf}
\end{figure}
\vspace{-3mm}
{\footnotesize Mortality rate for adults $65+$ and Progresa penetration, highly marginalized municipalities, 1991--2006. Vertical dashed line = 1997 (program rollout). Sample: municipalities with high (grade 4) or very high (grade 5) 1990 CONAPO Marginalization.}
\end{frame}
...
\textbf{Sources of variation:} following \smallcite{parker2023conditional}
...
\begin{itemize}
  ...
  \item $\textit{Post}_{t}$: $= 1$ for years $1997$--$2006$
  \item $\delta_m$, $\gamma_t$: municipality and year fixed effects
...
\textbf{Event study} to trace dynamic effects and test pre-trends:
...
\begin{frame}[shrink=15]{Main Results: Effect on Elderly Mortality Rate}\label{main_results}
\vspace{0.3cm}
\begin{center}
\begin{table}
  \centering
  ...
  \begin{threeparttable}
    ...
    \begin{tablenotes}
      \tiny
      \item \textit{Notes:} Outcome: deaths per 1,000 inhabitants aged $65+$. Columns (1)--(2): unweighted; columns (3)--(4): weighted by elderly population. ``SP'' = Seguro Popular intensity control. Municipality and year FEs in all columns. SE clustered at municipality level. $^{*}p<0.1$, $^{**}p<0.05$, $^{***}p<0.01$.
    \end{tablenotes}
  \end{threeparttable}
\end{table}
\end{center}
...
\small
\begin{itemize}
  \item[$\circ$] \textbf{Motivation:} prior work suggests mortality reductions through \textit{specific} causes (diabetes, infectious disease) \smallcite{BarhamRowberry2013JDE}
  \vspace{4pt}
...
\begin{itemize}
  \item[$\circ$] Main result (levels) is robust across alternative functional forms:
    ...
    \begin{enumerate}
      ...
      \vspace{4pt}
      \item \textbf{Poisson PPML with raw death counts} --- handles zero-mortality municipality-year cells; addresses \smallcite{ChenRoth2024QJE} concerns about log with zeros
      \vspace{4pt}
...
\begin{columns}[T]
  \begin{column}{0.52\textwidth}
    ...
    \vspace{4pt}
    \begin{itemize}
      \item[\textbf{(A)}] BR original: $\approx -4.0$ deaths/1,000 ($p<0.05$)
      \item[\textbf{(B)}] Our close replication: $-3.96$ deaths/1,000
      \vspace{4pt}
      \item[\textbf{(C)}] Add \textit{population weights}: estimate attenuates $\approx$\textbf{50\%}, loses precision
      \vspace{4pt}
      \item[\textbf{(D)-(E)}] Alternative lag structures (1-year, 3-year): \textbf{further attenuate} the estimate
    \end{itemize}
  \end{column}
  \begin{column}{0.46\textwidth}
    ...
    \begin{itemize}
      ...
      \item[$\circ$] Population weighting is equally or more defensible
    \end{itemize}
  \end{column}
\end{columns}
...
\note{We replicated the Barham and Rowberry result almost exactly — a mortality reduction of about 4 deaths per 1,000. But when we add population weights, the estimate cuts in half and becomes imprecise. When we use a one-year or three-year lag instead of two years, it shrinks further. These are all equally defensible choices. The key lesson is that the short-run positive finding is fragile.}
...
\begin{itemize}
  ...
  \item[$\circ$] \textbf{BR sample, unweighted:} estimate $\approx -4.0$ deaths/1,000 (similar magnitude to BR)
  \item[$\circ$] \textbf{BR sample, with population weights:} $-1.73$ deaths/1,000 $\to$ not significant
  ...
  \item[$\circ$] \textbf{Event study reveals additional concern for BR sample:}
    \begin{itemize}
      ...
      \item Neither sex supports the $-4.0$ pooled estimate
    \end{itemize}
  \vspace{6pt}
  \item[$\circ$] \textbf{Conclusion:} short-run effect, if real, was at most $\approx -1.7$ deaths/1,000 at full intensity; our long-run analysis finds no significant effect
\end{itemize}

\end{frame}

\note{When we apply our pre/post framework to the BR sample, we can now run the event study they couldn't. And we find that for males in their sample, there are clear pre-trends — meaning the identification assumption was likely violated. For females, the post-period doesn't show a consistent break. The weighted, defensible estimate is about -1.7 deaths per 1,000, not -4, and it's not significant. And in the full highly marginalized sample, there's no significant effect at all.}
...
\begin{table}
  ...
  \begin{threeparttable}
    ...
    \begin{tablenotes}
      \tiny
      \item \textit{Notes:} DiD estimates. Col.\ (1): weekly hours worked. Cols.\ (2)--(4): probability of living alone, with children, or with elderly only. Baseline: 1997. SE clustered at municipality level. $^{*}p<0.10$, $^{**}p<0.05$, $^{***}p<0.01$.
    \end{tablenotes}
  \end{threeparttable}
\end{table}
\end{center}
...
\note{Using the experimental data, we test directly whether Progresa changed elderly labor supply or living arrangements. The answer is no on both counts. Elderly people didn't work fewer hours, and they didn't become more or less likely to live with children or alone. This rules out the caregiving pathway as an operative mechanism.}

\begin{frame}{Mechanisms: Income and Nutrition (ENIGH)}\label{mechanisms_enigh}
\small
\begin{itemize}
  \item[$\circ$] \textbf{Data:} National Household Income and Expenditure Survey (ENIGH), 1992--2006
  \item[$\circ$] \textbf{Design:} same two-phase DiD as mortality analysis (equation 1)
  \item[$\circ$] \textbf{Outcomes:} individual labor income; household food expenditure by category (cereals, meat/dairy, vegetables, fats/sugar); out-of-pocket health spending
  \vspace{6pt}
  \item[$\circ$] \textbf{Results:} all estimated coefficients are imprecise with large standard errors
    \begin{itemize}
      \item Cannot detect any significant behavioral responses in income, nutrition, or health spending
    \end{itemize}
  \vspace{6pt}
  \item[$\circ$] \textbf{Interpretation:}
    \begin{itemize}
      \item Null ENIGH results are \textit{consistent} with the experimental null from the labor/living arrangements analysis
      \item Two independent data sources converge: income, nutrition, and caregiving channels not activated at detectable levels
      \item ENIGH was not designed to capture elderly subgroups in small rural municipalities $\to$ limited power, but direction is consistent
    \end{itemize}
\end{itemize}
\end{frame}

\note{Looking at the ENIGH household survey, we find no significant effects on labor income, food consumption by category, or out-of-pocket health spending. The estimates are imprecise, so we can't make strong claims from this alone. But the direction is consistent with the experimental null. Taken together, the income and nutrition channels don't appear to have been activated in ways large enough to reduce mortality.}
...
\begin{itemize}
  ...
  \item[$\circ$] \textbf{Policy implication:} CCTs structured around children's education do not reliably deliver mortality benefits to elderly household members
    \begin{itemize}
      \item Programs transferring resources \textit{directly} to the elderly are more effective for reducing elderly mortality \smallcite{jeon2025short, MiglinoNavarrete2023AEJP}
    \end{itemize}
  \vspace{4pt}
  \item[$\circ$] \textbf{Future work:} examine effects on elderly health outcomes with panel microdata; explore whether more recent program expansions (post-2006 pension component) changed these conclusions
\end{itemize}
\end{frame}
