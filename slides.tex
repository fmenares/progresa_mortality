...
\begin{columns}[T]
  \begin{column}{0.52\textwidth}
    ...
    \begin{itemize}
      ...
      \item[\textbf{(B)}] Our close replication: $-7.06$ deaths/1,000
      \vspace{4pt}
      \item[\textbf{(C)}] Add \textit{population weights}: attenuates $\approx$\textbf{47\%} to $-3.74$ deaths/1,000
      \vspace{4pt}
      \item[\textbf{(D)}] 1-year lag $+$ weights: $-3.46$ deaths/1,000
      \vspace{4pt}
      \item[\textbf{(E)}] 3-year lag $+$ weights: $-2.07$ deaths/1,000 ($p<0.10$ only)
    \end{itemize}
  \end{column}
