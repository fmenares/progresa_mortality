...
| IADB-2 | Eduardo | Endogeneity of treatment intensity (higher intensity ↔ deeper poverty ↔ worse mortality). | **Addressed** | Conf. Comments 3, 4 |
| IADB-3 | Óscar | "Forbidden comparisons" / staggered adoption (Callaway–Sant'Anna); two-period intensity fix may not solve it. | **NEW — not started** | — |
...
**IADB-2 (Eduardo) — intensity endogeneity.** *Comment:* higher-intensity municipalities are likely poorer, and poverty predicts worse mortality → bias. *Our response:* (i) restrict to highly-marginalized municipalities (baseline comparability); (ii) include Intensity_2005 to absorb within-group wealth heterogeneity. *Assessment:* Already core to the design; also directly supported by the `AT_ses_trend` robustness (trend × baseline marginalization) and the Intensity_1999/2005 correlation discussion (Conf. Comments 3–4). Largely a "point to existing robustness" response; no new work needed beyond making sure the text foregrounds it.

**IADB-3 (Óscar) — staggered adoption / forbidden comparisons (the most-debated point).** *Comment:* fixing intensity at just 1999 and 2005 to handle staggered rollout does **not** clearly solve the Callaway–Sant'Anna forbidden-comparison problem — real intensity also varied in 2000–2004, and those intermediate cohorts still contaminate already-treated vs. newly-treated comparisons. Óscar was tentative ("I need to think more, but ignoring that intensity keeps changing doesn't remove the staggered-adoption problem") and offered two concrete robustness paths: **(A)** Callaway–Sant'Anna with **continuous** treatment at the municipality level (noted the continuous-DiD literature is moving fast; municipal intensity is computable even though original assignment was at the unobserved *locality* level); **(B)** **binarize** treatment (e.g. ≥15% intensity = treated, ~0 = control) and run a standard year-by-year event study — called this "super clean" and "low-hanging fruit." *Our response:* accepted both as reasonable robustness; argued current spec loads intermediate heterogeneity onto the 2005 coefficient and the paper's relevant variation is the 1999 phase. *Assessment:* **This is the most important new to-do.** Option B is genuinely low-cost and we already have event-study machinery (`AF_beta1_*`, `AF_ses_trend`) to adapt — define a binary `treated = inten1999 >= 0.15` (threshold to sensitivity-check) and run an event study on the existing panel. Option A (continuous-treatment CS, e.g. `csdid`/`did_continuous`) is more involved but is the modern-standard rebuttal and would materially strengthen the identification section. Recommend implementing **B first** (fast, clean), then **A** as the heavier robustness. Flag: threshold choice for B needs a sensitivity grid; and CS-continuous needs a defined "cohort/timing" variable, which is awkward when treatment is an intensity that ramps rather than a clean switch-on — worth thinking through before coding.
...
- **Highest-value new work: IADB-3 (staggered adoption).** Binarized event study (Option B) is low-cost, uses existing event-study code, and directly answers the single most-debated critique. Continuous-treatment Callaway–Sant'Anna (Option A) is the stronger, heavier follow-up. This is the one that could most change how referees view identification.
- **Cheap, complementary: IADB-4 + IADB-5** — size-tercile and vulnerability-share subsample tables; do together.
- **Mostly "point to existing robustness": IADB-2, IADB-7** (and IADB-6's Seguro-Popular part) — largely a writing task, with IADB-7 needing a sentence on the long-run migration sign-flip.
