...
- **Per user instruction, all figure/table output is now wired into the appendix LaTeX files** (previously only `di` log output for D0's R² and D3's diagnostics): `figures_app.tex` gained two new figures (`af:pv_fig3_replication`, `af:beta0_stability`, following the established caption/note style, plain-text convention for in-table variable labels — avoided `\textsubscript`, which isn't used/confirmed elsewhere in this document, in favor of the existing `$\textit{Intensity}_{1999}$` math-mode convention); `tables_app.tex` gained two new tables (`at:pv_fig3_r2`, `at:saturation_diagnostics`). Checked: table/figure environments remain balanced (17 begin/end table, 10 begin/end figure) and no duplicate labels were introduced.
- **Status: code written, NOT run against real data** (no Progresa/mortality data in this sandbox). User needs to run `02_mortality.do` and report back: (a) the `phase2_new<0` count (validates the cumulative-snapshot assumption); (b) the β₀-stability event study itself — do the four year-by-year profiles overlay closely?; (c, d — user will check later per their own instruction) the D3 stayer share and first-crossing-year spread — this is the number that finally settles the dCDH-feasibility question, replacing the earlier five-snapshot approximation.
