...
**Asked user to check** (not yet resolved): (1) whether a companion count variable exists elsewhere in the file (e.g., `m150*` or similar, not in the original variable list shared) via `describe m150* m149*, fullnames`; (2) if genuinely absent, which harmonized outcome to pool on instead — e.g., a binary "any utilization" indicator for both waves, or "number of distinct service types used" (which would mean coarsening the November wave's true counts down to match). This is a substantive definitional choice, not a coding detail, and changes what the pooled Gertler-style regression estimates. **No pooling code has been written yet — waiting on this before implementing**, to avoid silently corrupting `total_visits`.
