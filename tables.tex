...
\floatbox[{\captop}]{table}[\FBwidth]
  ...
  {\scalebox{0.85}{
    ...
    \vspace{-3mm}
    \floatfoot{\textit{Notes:} This table reports difference-in-differences estimates of the short-run effect of Progresa on labor supply, living arrangements, and health-care utilization for elderly individuals aged 65 and older in eligible households. The sample is restricted to households eligible for Progresa. Column (1) reports weekly hours worked. Columns (2) and (3) report weekly hours worked restricted to \textit{elderly-only} households (those with no children or working-age adults present); these households receive only the direct food transfer (\textit{apoyo alimentario}), not the child-conditional education grants (\textit{apoyo educativo}), and thus isolate the labor-supply response to a direct transfer to the elderly. $^{\S}$ Column (2) defines elderly-only status contemporaneously in each round; $^{\P}$ column (3) fixes household composition at its 1997 (pre-treatment) value to avoid conditioning on a post-treatment outcome. Columns (4)--(6) report living arrangement outcomes: living alone, living with children, and living only with other elderly members, respectively. Columns (7)--(8) report total health-facility visits in the past four weeks (1999 cross-section, municipality fixed effects) for the age 65+ and age 51+ samples; the latter is comparable to Gertler (2000), Table 6. The baseline year is 1997. Standard errors are clustered at the municipality level and shown in parentheses. $^{*}$ $p < 0.10$, $^{**}$ $p < 0.05$, $^{***}$ $p < 0.01$.}
  }
\end{table}