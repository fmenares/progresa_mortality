...
%------------------------------------------------------------
% LIVE figure, generated by codes/02_mortality.do (block D2, immediately
% after AF8_beta0_stability). PROVISIONAL -- the PDFs below are
% placeholders pending a real Stata run; swap in the real
% AF10a/b/c_no_control_*.pdf outputs and drop the \textcolor{red}{} once
% reviewed.
%------------------------------------------------------------
\textcolor{red}{
\begin{figure}[H]
...
{\includegraphics[width=0.49\textwidth]{figures/appendix/AF10c_no_control_m.pdf}}
         ...
         \floatfoot{Notes: PROVISIONAL -- pending a Stata run. This figure presents event-study estimates of the interaction between $\textit{Intensity}_{1999}$ and year indicators (reference year 1996) for the pooled, female, and male older-adult (65+) mortality rate per 1,000 inhabitants, corresponding to the ``No control'' series of Appendix Figure~\ref{af:beta0_stability} -- estimated there pooled only -- now disaggregated by sex. Unlike the baseline event studies (e.g.\ Figure~\ref{f:es_dd_sex_no_weight}), no later-phase (cumulative enrollment) intensity control is included, illustrating the misspecification that arises when later cumulative enrollment is left uncontrolled for. All specifications include Seguro Popular intensity, municipality and year fixed effects, and are weighted by the population aged 65 and older. The vertical line marks the program's 1997 rollout. The sample is restricted to highly marginalized municipalities. 95\% confidence intervals shown.}
\end{figure}
}
