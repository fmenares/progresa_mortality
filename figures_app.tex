...
{\includegraphics[width=0.49\textwidth]{figures/appendix/AF5a_tb_card_Marg.pdf}}
    ...
    \subfigure[Accidents]
     {\includegraphics[width=0.49\textwidth]{figures/appendix/AF5d_tb_accid_Marg.pdf}}
         ...
         \floatfoot{Notes: Each panel displays interaction coefficients between Progresa intensity in 1999 and year indicators for the indicated cause of death. The outcome is the mortality rate for adults aged 65 and older per 1,000 inhabitants attributable to each cause. The specification includes municipality and year fixed effects. Standard errors are clustered at the municipality level. The reference year is 1996. The sample is restricted to highly marginalized municipalities. Regressions are weighted by population 65+. 95\% confidence intervals shown.}
\end{figure}
...
{\includegraphics[width=0.85\textwidth]{figures/appendix/AF8_beta0_stability.pdf}}
         ...
         \floatfoot{Notes: This figure presents event-study estimates of the interaction between $\textit{Intensity}_{1999}$ and year indicators (reference year 1996) for the older-adult (65+) mortality rate per 1,000 inhabitants, under four specifications. The first, ``No control,'' omits any later-phase intensity control entirely, illustrating the misspecification that arises when later cumulative enrollment is left uncontrolled for. The remaining three add a later-phase (later cumulative enrollment) control: the baseline specification controlling for $\textit{Intensity}_{2005}$, and specifications replacing it with the adjacent $\textit{Intensity}_{2004}$ and $\textit{Intensity}_{2006}$ snapshots. All specifications 
         include Seguro Popular intensity, municipality and year fixed effects, and are weighted by the population aged 65 and older. The vertical line marks the program's 1997 rollout. The sample is restricted to highly marginalized municipalities. 95\% confidence intervals shown.}
\end{figure}
...
%------------------------------------------------------------
% LIVE migration-robustness figure, generated by
% codes/04_extra_robustness.do (see migration_robustness_summary.md).
% PROVISIONAL -- not yet run, and not yet decided whether it stays in
% the paper.
%------------------------------------------------------------
...
{\includegraphics[width=0.49\textwidth]{figures/appendix/AF_migration_es_poisson.pdf}}
         ...
         \floatfoot{Notes: PROVISIONAL -- see migration\_robustness\_summary.md before citing. \textbf{Panels (b) Log and (c) Poisson are pending a rerun of codes/04\_extra\_robustness.do and are not shown in this compile.} This figure presents event-study estimates of the interaction between $\textit{Intensity}_{1999}$ and year indicators (reference year 1996) for the 65+ population, pooled, female, and male, as a check on whether any apparent migration effect (Appendix Tables~\ref{at:migration_rob}--\ref{at:migration_rob_agefe}) is a pre-existing trend rather than a post-1997 program effect: if the profile shows a differential trend before 1997, it cannot be attributed to \textit{Progresa}. Panel~(a) uses the population count, corresponding to column~(1) of Appendix Table~\ref{at:migration_rob}; panel~(b) (pending) uses log population, corresponding to its column~(2); panel~(c) (pending) uses Poisson pseudo-maximum likelihood, corresponding to its column~(3), with coefficients computed as $(\exp(\hat\beta_t)-1)\times 100$, the percentage change in population at each year relative to 1996. Same specification as the mortality event studies (Figure~\ref{f:es_dd_sex_no_weight}): municipality and year fixed effects, Seguro Popular intensity, clustered at the municipality level. The sample is restricted to highly marginalized municipalities. The \texttt{\$mig\_intensity}/\texttt{\$mig\_years} switches described in Appendix Table~\ref{at:migration_rob}'s note apply here; under \texttt{\$mig\_years = "census"} none of the three panels is produced, as too few years remain to trace a profile. 95\% confidence intervals shown.}
\end{figure}

% Panels (b) Log and (c) Poisson are commented out because their source
% PDFs (AF_migration_es_log.pdf, AF_migration_es_poisson.pdf) do not exist
% on disk yet -- codes/04_extra_robustness.do has not been rerun since
% those panels were added to the code, so \includegraphics on either would
% be a hard compile error. Restore both subfigure blocks below (and widen
% panel (a) back to 0.49\textwidth is already correct) once the next
% Stata run produces both files, and update the note back to describing
% all three panels (current note already anticipates this and describes
% panels (b)/(c); revert to that version, not the one active now).
